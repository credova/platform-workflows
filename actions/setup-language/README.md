...
- Node caches `~/.npm` when `package-lock.json`/`yarn.lock` exists and `~/.bun/install/cache` when `bun.lock` exists (pinned via `BUN_INSTALL_CACHE_DIR`). pnpm is not cached.
